...
# PROMPT_SUMMARY = """
# 你是一名出行达人，也是一名NLP专家，请给我总结以下文本，生成一份通俗易懂的自然语言
# ```text
# {{query}}
# ```
# ## 要求
# - 文本中可能包含天气、火车票、周边娱乐餐饮景点信息，请用自然语言总结
# - 按以下格式返回内容
# ```text
...
# - 用户输入
# ```text
# user: 根据给定的文本，天气情况为多云，温度为11摄氏度，湿度为91%。建议出门时携带外套，因为温度较低，同时注意保暖。
# 根据给定的文本，我为您提供以下建议：

# 1. 针对第一段文本，我建议您前往以下餐厅：三口井头一家烧烤、尚品宫时尚自助餐厅、五小阿婆烧烤、大富烧鸡(恒基广场店)和罗婆烧烤(一分店)。这些餐厅的性价比最高，地址分别为中山一路太平街龙亭巷114号、中山一路新兴路21号、百胜街田垌巷27号、新兴路18号恒基广场F1层和爱新街77号。您可以联系这些餐厅的电话号码进行预订或了解更多信息。

# 2. 针对第二段文本，我建议您入住以下酒店：拉斐皇廷商务酒店、红山国际大酒店、雅斯特酒店(百色五中体育中心高铁站店)、城市便捷酒店(那坡汽车总站店)和星心桥酒店(百色星河水上乐园店)。这些酒店的性价比最高，地址分别为新百通金穗大道37号、德盛路和红山路交汇处、竹洲大道莲塘
# 村林屋屯62号、城厢镇幸福新城伏必二组预留安置地4号和纳贤路都市阳光欢乐街区星河水上乐园旁。您可以联系这些酒店的电话号码进行预订或了解更多信息。

# ```

# - 返回
# ```text
# 今天天气多云，温度大约11度，湿度挺高的，有91%，所以出门记得带件外套，别着凉了。如果你打算去吃饭，可以去三口井头一家烧烤、尚品宫时尚自助餐厅、五小阿婆烧烤、大富烧鸡(恒基广场店)或者罗婆烧烤(一分店)，这些地方性价比高，地址和电话我都有，你可以提前打电话预订。晚上想住酒店的话，拉斐皇廷商务酒店、红山国际大酒店、雅斯特酒店(百色五中体育中心高铁站店)、城市便捷酒店(那坡汽车总站店)和星心桥酒店(百色星河水上乐园店)都是不错的选择，同样提供地址和电话，方便你预订。
# ```
# """
...
class GetTransInfo(Action):
    ...
    async def run(self, msgs):
        ...
        # print(resp_weather)
        # print(resp_keywords)
        # print(f"resp_ticket: {resp_ticket}")
        # resp = f"""{resp_weather}\r\n{resp_keywords}\r\n{resp_ticket}"""
